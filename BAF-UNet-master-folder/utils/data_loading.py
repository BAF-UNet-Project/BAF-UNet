...
#     if len(mask_file) == 0:
#         raise FileNotFoundError(f"No mask found for {idx}")
...
#         # ✅ FIX: NO MULTIPROCESSING (Windows safe)
#         logging.info('Scanning mask files to determine unique values')
#         unique = [
#             unique_mask_values(x, self.mask_dir, self.mask_suffix)
#             for x in self.ids
#         ]
...
#         assert len(img_file) == 1, f'Image issue for {name}'
#         assert len(mask_file) == 1, f'Mask issue for {name}'

#         mask = load_image(mask_file[0])
#         img = load_image(img_file[0])

#         assert img.size == mask.size, f'Size mismatch for {name}'
